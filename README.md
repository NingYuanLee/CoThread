...
左侧「本地连接器」可下载免安装的 Windows 单文件绿色版并查看自己的设备。首次双击后，连接器先检测 Git 和 Codex CLI，再打开共序网页，由用户登录并确认设备授权；授权后以一行一个项目的方式列出该账号加入的项目。一个账号同一时间只保留一台连接器，重新授权另一台电脑会替换旧连接；这一台连接器可同时绑定多个项目。Codex CLI 的账号登录状态仅作提示，不阻止使用本机已配置的自定义模型厂商。用户可为每个项目分别选择本地 Git 根目录、是否允许 Git 推送并开启连接。本地路径和上述本机配置仅保存在电脑上，云端不保存路径或上传仓库。
...
构建 Windows 单文件程序使用 `npm run connector:build`。程序无需安装，运行数据写入当前用户的 `%LOCALAPPDATA%\\CoThreadConnector`；“开机自动启动”使用当前用户启动项，无需管理员权限。发布前设置 `CONNECTOR_UPDATE_PUBLIC_KEY` 写入更新验签公钥；设置 `CONNECTOR_CODESIGN_CERT_SHA1` 可在 SEA 注入后对最终 EXE 做 Authenticode 签名。随后用 `CONNECTOR_UPDATE_PRIVATE_KEY` 执行 `npm run connector:sign -- <exe> <version> <download-url>`，把输出的版本、SHA-256 和 Ed25519 签名配置到 Makers 的 `CONNECTOR_RELEASE_*` 环境变量。私钥和代码签名证书不能进入仓库。
