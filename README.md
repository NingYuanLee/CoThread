...
这是独立于 `D:\work\dsh` 的协作应用。项目是长期工作空间，一次迭代对应一个群聊。成员用自己的本地 AI 读取上下文、提交产物，团队人工审核后归档。Makers 云端使用原生沙箱，本地独立部署保留 ACS 适配；MySQL 是业务数据和附件的持久化存储。
...
- 主助手只负责接待、澄清、分派和回答进度，没有执行工具；最多同时运行 3 个临时执行子 Agent，主助手不占执行名额。同一成员再次 @ 补充执行要求时更新已有任务，通过 DSH 原生 steering 在下一执行步骤接收，不新建任务；进度询问直接由主助手回答。满额时新成员的执行请求等待空位。聊天名称旁显示「主助手」或「子 Agent 1／2／3」，编号在任务完成后可复用。
- 子 Agent 使用独立 DSH 上下文与工作目录，完成后释放进程及执行名额；Makers 同一会话共享原生沙箱，子任务不会关闭其他任务使用的实例。正文、附件引用、已发布成果和执行记录保留。各任务可单独停止，更新消息与原任务关联。
- 每个迭代独立 Agent 会话；会话快照、工具过程与已发布产物入 MySQL。Makers 沙箱按需创建，工作目录使用平台检查点持久化，长期成果仍需发布到文档库。
...
- 成员发言增量纳入主上下文，每条纳入后检查 900K 自动压缩阈值；上下文同步、压缩与主助手接待分开执行。子 Agent 继承主上下文保留的摘要和消息，在独立上下文中继续工作。
