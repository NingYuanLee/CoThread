...
- 小祥采用三级逻辑层：项目级一级小祥昵称“老翁”、名称“项目知识库管理员”，每个迭代的二级小祥是任务调度员，每项工作的三级小祥是任务执行者。聊天中三个层级对用户统一显示为“小祥”，监控页展示内部昵称和职责。一级小祥按四类维护任务拆成独立 DSH 会话：成员发言、文档摘要、文档整理、迭代归档均按项目复用；关系库仍保存成员认识、文档摘要、项目长期总结、权限和任务状态等权威记录。
