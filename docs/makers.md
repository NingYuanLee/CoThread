...
- 成员发言、成员被 @ 和新文档版本只实时写入 MySQL 待整理队列，不同步调用一级小祥。`/cothread-memory` 与主站 `POST /api/memory-maintenance` 是同一套知识库维护入口（Bearer `MEMORY_MAINTENANCE_TOKEN`），每次最多处理 25 个到期批次后退出；在 Makers 控制台配置每分钟定时调用任一入口。手动「整理成员认识 / 文档摘要」会在入队后于当前请求内直接派发处理（Makers 云函数没有进程内唤醒 worker）。常驻 Node 部署另有 60 秒内置扫描。超时卡住的 `running` L1 任务会在维护入口自动重新入队。队列与处理游标保证实例回收、失败重试和跨迭代乱序完成时不丢更新。
