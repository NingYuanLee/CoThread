...
- 小祥采用三级逻辑层：项目级一级小祥昵称“老翁”、名称“项目知识库管理员”，每个迭代的二级小祥是任务调度员，每项工作的三级小祥是任务执行者。聊天中三个层级对用户统一显示为“小祥”，监控页展示内部昵称和职责。每个项目拥有独立、可恢复的 L1 DSH 会话；关系库仍保存成员认识、文档摘要、权限和任务状态等权威记录。
