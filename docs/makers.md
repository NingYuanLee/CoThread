...
- 小祥采用三级逻辑层：项目级一级小祥昵称“老翁”、名称“项目知识库管理员”，每个迭代的二级小祥是任务调度员，每项工作的三级小祥是任务执行者。聊天中三个层级对用户统一显示为“小祥”，监控页展示内部昵称和职责。一级层不依赖常驻模型上下文，未来可为持久化记录增加向量索引，但关系库仍保存权威记录。
...
原生文件接口按文本使用，二进制文档先分块传输再在沙箱内解码；模型不会收到这些传输数据。工具桥接保留请求的原生沙箱上下文。若平台没有注入该上下文，工具明确报错，不回退连接已删除的 ACS。小祥的普通接待、数据库读取和 DSH 上下文维护不因缺少 ACS 配置而被禁用。
