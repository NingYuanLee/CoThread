...
本地连接器由系统外部渠道分发。首次运行通过 Makers 网页登录授权，随后由连接器读取账号项目列表；本地仓库路径、项目开关、推送权限和开机启动设置不上传。Makers 只保存设备、授权、项目级连接权限和任务状态，不保存连接器安装包，也不提供版本发布、下载或自动更新能力。
...
- 小祥采用三级逻辑层：项目级一级小祥昵称“老翁”、名称“项目知识库管理员”，每个迭代的二级小祥是群成员式调度员（快速回应，重活交给三级，交活后再向群回报），每项工作的三级小祥是任务执行者（结束前必须向二级交活）。聊天中三个层级对用户统一显示为“小祥”，监控页展示内部昵称和职责。一级小祥按四类维护任务拆成独立 DSH 会话：成员发言、文档摘要、文档整理、迭代归档均按项目复用；关系库仍保存成员认识、文档摘要、项目长期总结、权限和任务状态等权威记录。
