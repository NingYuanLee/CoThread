...
export async function reopenConnectorTask(conn, connectorTaskId, { publish = false } = {}) {
  ...
  if (!ownedByConnector || !["cancelled", "failed"].includes(task.status)) return syncConnectorTask(conn, connectorTask, { publish });
  await query(conn, `UPDATE agent_tasks SET status='queued',progress=?,result_summary=NULL,finished_at=NULL,revision=revision+1 WHERE id=?`,
  ...
  await recordTaskStatusChange(conn, task.id, task.status, connectorActor(connectorTask), "本机连接器重试，任务重新排队");
