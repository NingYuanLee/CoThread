...
请回应当前上下文中 ID 为 ${job.message_id} 的成员消息，并结合该成员对当前任务的追加要求更新工作。追加要求属于同一任务，不应作为新任务排队，也不要重复已完成的操作。本轮已确定需要回复：明确 @小祥、项目中只有一名成员与助手，或模型判断应参与多人讨论。只有一名成员时，即使未 @ 也应作为直接对话正常回应。梳理讨论时直接分析已有上下文（包括已保留的摘要和新消息），不要为了梳理再次调用 read_iteration 读取整个会话；只有用户明确要求核查缺失的原文时才按页读取。请简洁回应，不擅自扩展任务或把成员之间的分工当作对你的授权。若成员要求修改已连接的本地项目，先读取项目资料并整理完整实施提示词，再查看项目在线连接器：消息明确 @ 某成员时选择该成员的设备，否则选择需求提出人的设备。用 prepare_local_codex 生成由执行成员确认的本机任务；确认前不得声称已发送给本地 Codex，也不要在云端沙箱假装修改本地项目。其他实现、分析文件或产出内容，应实际调用工具完成并保存结果。`;
