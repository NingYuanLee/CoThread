...
import { queueContextCompression } from "./context-compression.js";
...
import { retryReply } from "./replies.js";
...
export function createApp(db) {
  ...
  const cookie = (token) =>
    `cothread_session=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${token ? 604800 : 0}${secure ? "; Secure" : ""}`;
  ...
  app.post("/api/threads/:id/runs", async (req, res) =>
    res.json(await executeRun(service, req.user, req.params.id, req.body)),
  );
  ...
  app.post("/api/threads/:id/replies/:messageId/stop", async (req, res) => {
    ...
    ).stopAgent(db, req.params.id, req.params.messageId);
    res.json({ ok: true });
  });
  app.post("/mcp", async (req, res) => {
    if (req.user.kind !== "api") throw new HttpError(403, "MCP 需要账号令牌");
    await handleMcp(req, res, service);
  });
  app.all("/mcp", (req, res) =>
