...
export async function attachMessageQuotes(db, messages) {
  ...
  const rows = await query(db, `SELECT q.message_id,m.id,m.thread_id,m.sequence,m.source,
    LEFT(m.body,500) body,u.name author,m.author_id,m.refs,m.folder_refs
  ...
  for (const row of rows) {
    ...
    quote.folder_refs = typeof quote.folder_refs === 'string' ? JSON.parse(quote.folder_refs) : (quote.folder_refs || []);
