...
export async function retryReply(service, user, threadId, messageId) {
  ...
  const result = await transaction(service.db, async (db) => {
    ...
    const result = await query(
      ...
      SET r.status='queued',r.error=NULL,r.finished_at=NULL,r.first_response_at=NULL,r.usage_stats=NULL
    ...
    await query(db, "UPDATE agent_requests SET status='queued',error=NULL,first_response_at=NULL WHERE message_id=? AND status='failed'", [messageId]);
    return { ok: true };
  });
