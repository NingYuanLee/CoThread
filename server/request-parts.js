...
export function registerRequestParts(app, db) {
  app.post("/api/request-parts/:id/:part", async (req, res) => {
    ...
    const part = z.coerce.number().int().min(0).max(63).parse(req.params.part);
    const data = z.object({ content: z.string().max(524288), total: z.number().int().min(1).max(64) }).parse(req.body);
    ...
    await transaction(db, async (conn) => {
      ...
      if (Number(usage.bytes) + Buffer.byteLength(data.content) > 128 * 1024 * 1024)
  ...
  app.use("/api", async (req, res, next) => {
    ...
    req.body = await transaction(db, async (conn) => {
      ...
      if (parts.reduce((n, p) => n + p.content.length, 0) > 32 * 1024 * 1024)
