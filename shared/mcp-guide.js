...
发送文件必须分两步：先调用 upload_source_file(threadId, title, filename, contentBase64, mime?) 上传到当前迭代的缓存文件并取得版本 ID，再调用 post_message(threadId, body, refs:[版本 ID], mentionAgent?) 发送消息。不要把文件放进 post_message.files；该字段仅为旧客户端保留，新的调用必须避免把大段 Base64 和消息一起提交。refs 必须是当前迭代或项目正式文件的版本 ID，不是文件名或 artifactId，等同 UI 的 /关联文件。单个文件上限 5 MiB，消息最多关联 30 个版本，body 最多 20000 字符。
...
你当前通过 CoThread 本机连接器启动。连接器负责本机任务生命周期和 Agent 配置同步；MCP 只负责项目协作。若连接器提示令牌版本已更新，请等待连接器更新本机 Agent 配置后重新打开 MCP 会话。`;

export function mcpInstructionsForSource(source) {
  return source === "connector_mcp" ? CONNECTOR_MCP_INSTRUCTIONS : MCP_INSTRUCTIONS;
}
...
工具清单：get_connection_guide、list_projects、get_project、get_iteration_context、get_document_version、upload_source_file、post_message、submit_document。
