...
test("chat views omit repeated binary data and lazy resources retain authorization", async () => {
  ...
  try {
    ...
    const startupResponse = await get(`/api/workspace?projectId=${project.id}&threadId=${thread.id}`);
    assert.equal(startupResponse.status, 200);
    assert.match(startupResponse.headers.get("server-timing"), /app;dur=[\d.]+.*db;dur=[\d.]+.*db_count;desc="\d+"/);
    assert.match(startupResponse.headers.get("x-cothread-request-id"), /^[a-f0-9-]{36}$/);
    const workspace = await startupResponse.json();
    assert.equal(workspace.user.id, actor.id);
    assert.equal(workspace.project.id, project.id);
    assert.equal(workspace.thread.id, thread.id);
    assert.equal(workspace.thread.messages.length, 12);
    assert.equal(workspace.health.status, "ok");
    const outsideWorkspace = await (await get(`/api/workspace?projectId=${project.id}&threadId=${thread.id}`, outsideSession.token)).json();
    assert.equal(outsideWorkspace.project, null);
    assert.equal(outsideWorkspace.thread, null);
    assert.deepEqual(outsideWorkspace.projects, []);
    const fallbackWorkspace = await (await get(`/api/workspace?projectId=stale&threadId=stale`)).json();
    assert.equal(fallbackWorkspace.project.id, project.id);
    assert.ok([thread.id, second.id].includes(fallbackWorkspace.thread.id));
