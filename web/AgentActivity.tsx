...
export function AgentActivity({threadId,messageId,events,output,status,hasFinal,versions,threads,progress,startedAt,finishedAt}:{
 ...
 useEffect(()=>{if(!running)return;setNow(Date.now());const timer=setInterval(()=>setNow(Date.now()),1000);return()=>clearInterval(timer);},[running,startedAt]);
 ...
 const current=!active?({completed:'已完成',failed:'未完成',cancelled:'已停止',queued:progress||'已收到'}[status]||'执行过程')
 ...
 const hasContent=ordered.some(e=>!(hasFinal&&e.tool==='assistant_final'))||!!output?.reasoning||(!hasFinal&&!!output?.content);
 if(!hasContent)return <div className="agent-trace agent-trace-empty"><div className="agent-status-line"><span className={running?'agent-current-step agent-step-active':'agent-current-step'} role="status">{current}</span>{elapsed&&<small className="agent-elapsed">{elapsed}</small>}</div></div>;
 return <details className="agent-trace" onToggle={e=>setOpen(e.currentTarget.open)}>
  ...
  {open&&<div className="agent-activity-items">
   ...
   {output?.event_id&&!ordered.some(e=>String(e.id)===String(output.event_id))&&active&&<div className="agent-phase"><div className="message-text"><Markdown remarkPlugins={[remarkGfm]} components={{img:()=> <span>（图片链接）</span>}}>{output.content||output.reasoning}</Markdown></div></div>}
